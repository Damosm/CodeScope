...
`Analysis` possède des `ProjectInfo`, qui possèdent des `CodeSymbol`, `ProjectReferenceInfo` et `PackageReferenceInfo`. Une analyse possède aussi des `CodeRelation`, `SourceFileInfo`, `RepositorySnapshot`, `SqlObject`, `SqlColumn`, `SqlReference`, `SqlColumnReference`, `OrmEntityMapping`, `OrmPropertyMapping`, `ApiEndpoint`, `CobolSymbol`, `CobolRelation` et `AnalysisDiagnostic`. Chaque relation référence sa source, éventuellement une cible interne, les noms qualifiés des deux extrémités, sa nature, sa confiance et l'emplacement de la première occurrence. Les clés sont des GUID. Les recherches sont indexées par projet, chemin et nom ; les relations, mappings et diagnostics le sont par analyse, source/cible ou niveau, avec suppression en cascade depuis l'analyse.
